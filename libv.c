// player_fixed.c - Intel macOS VideoToolbox 硬件解码版（真·流畅）
// 编译命令：
...
typedef struct {
    ...
    SwrContext *swr_ctx;
    AVBufferRef *hw_device_ctx; // 👈 新增：硬件解码上下文
...
static void *video_decode_thread(void *arg) {
    ...
    while (!atomic_load(&ctx->quit)) {
        ...
        while ((ret = avcodec_receive_frame(ctx->video_ctx, ctx->video_frame)) == 0) {
            ...
            // 🚀 核心：如果是 VideoToolbox 硬件帧，安全转换到内存
            if (ctx->video_frame->format == AV_PIX_FMT_VIDEOTOOLBOX) {
                sw_frame = av_frame_alloc();
                sw_frame->format = AV_PIX_FMT_NV12; // VT 标准输出格式
                ...
                if (av_hwframe_transfer_data(sw_frame, ctx->video_frame, 0) < 0) {
                    fprintf(stderr, "[WARN] HW->SW transfer failed, dropping frame\n");
                    av_frame_free(&sw_frame);
                    av_frame_unref(ctx->video_frame);
                    continue;
                }
                display_frame = sw_frame; // 后续使用转换后的软帧
            }

            // 动态重建 SwsContext (NV12/YUV -> RGB24)
...
int main(int argc, char *argv[]) {
    ...
    // 🚀 启用 VideoToolbox 硬件解码（Intel Mac 流畅核心）
    if (av_hwdevice_ctx_create(&ctx.hw_device_ctx, AV_HWDEVICE_TYPE_VIDEOTOOLBOX, NULL, NULL, 0) == 0) {
        ...
        fprintf(stderr, "[HW] VideoToolbox unavailable, fallback to software\n");
    }

    AVDictionary *vo = NULL;
    // 不再禁用 hwaccel，交由 FFmpeg 自动协商
